...
{
	public class ComapnyForgotPwCommandHandler : IRequestHandler<ComapnyForgotPwCommand, bool>
	{
		private readonly IAuthRepo _authRepo;
		private readonly IEmailService _emailService;

		public ComapnyForgotPwCommandHandler(IEmailService emailService, IAuthRepo authRepo)
		{
			_emailService = emailService;
			_authRepo = authRepo;
		}

		public async Task<bool> Handle(ComapnyForgotPwCommand request, CancellationToken cancellationToken)
		{
			try
			{
				var company = await _authRepo.GetCompanyByEmail(request.Email);
				if (company == null) throw new Exception("User not found");
				...
				if (res)
				{
					...
					userEntity.Name = company.Name;
					userEntity.Email = company.Email;
					userEntity.Password = company.Password;
					userEntity.Otp = otp;
